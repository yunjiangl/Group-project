...
function formMenu(bill_id, bill_pay_type, bill_money) {
	$("#contentDiv").remove();
	ajaxRequest("GET", getRootPath_web() + "/formMenu.html", null, function(data) {
		$("#bigDiv").append(data);
...
function analyze() {
	$("#contentDiv").remove();
	ajaxRequest("GET", getRootPath_web() + "/analyze.html", null, function(data) {
		$("#bigDiv").append(data);

		var pieData; // 饼图数据

		// 加载饼图后台数据
		ajaxRequest("GET", getRootPath_web() + "/pieData", null, function(billMoney) {
		...
		// 加载后台收入曲线图date数据
		ajaxRequest("GET", getRootPath_web() + "/incomeDate", null, function(billDate) {
			incomeDate = JSON.parse(billDate);
		});

		// 加载后台收入曲线图money数据
		ajaxRequest(type, getRootPath_web() + "/incomeMoeny", data, function(billMoney) {
		...
		// 加载后台支出曲线图date数据
		ajaxRequest("GET", getRootPath_web() + "/payDate", null, function(billDate) {
			payDate = JSON.parse(billDate);
		});

		// 加载后台支出曲线图money数据
		ajaxRequest(type, getRootPath_web() + "/payMoeny", data, function(billMoney) {
...
function addBill(bill_pay_type, bill_money) {
	ajaxRequest("GET", getRootPath_web() + "/addBill", "bill_pay_type=" + bill_pay_type() + "&bill_money=" + bill_money, function() {
		alert("新增成功！");
...
function compileBill(bill_id, bill_pay_type, bill_money) {
	ajaxRequest("GET", getRootPath_web() + "/updateOrSaveBill", "bill_id=" + bill_id + "&bill_pay_type=" + bill_pay_type + "&bill_money=" + bill_money, function(msg) {
		alert(msg);
		$(location).attr('href', 'list.html');
	});
}
...
function deleteBill(bill_id) {
	if (confirm("您确定删除吗？")) {
		ajaxRequest("GET", getRootPath_web() + "/deleteBill", "bill_id=" + bill_id, function() {
			alert("删除成功！");
			$(location).attr('href', 'list.html');
		});
	}
...
function editOrDelete() {
	...
	// 删除
	$(".deleteDiv").click(deleteBill($(this).parent().find("input").val()));
}
 ...
 * 
 **/
function countForPagesAndFirstData(url, bill_pay_type) {
	var queryUrl = getRootPath_web() + url;
	var nextUrl;
	ajaxRequest("GET", queryUrl, "bill_pay_type=" + bill_pay_type, function(data) {
		pages = data;
		$(".pages").remove();
		...
		for (i = 0; i < pages; i++) {
			$("#previous").before("<button class='pagingButton pages'>" + (i + 1) + "</button>");
		}
		...
		$(".pages").click(function() {
			...
			if ($("#searchInput']").val() != "") {
				if ($("#payIncome_Select").val() == "收入查询") {
					nextUrl = getRootPath_web() + "/inquireIncomeData";
				} else {
					nextUrl = getRootPath_web() + "/inquirePayData";
				}
			} else {
				if ($("#payIncome_Select").val() == "收入查询") {
					nextUrl = getRootPath_web() + "/incomeData";
				} else {
					nextUrl = getRootPath_web() + "/payData";
				}
			}

			ajaxRequest('GET', nextUrl, "page=" + nextPage + "&bill_pay_type=" + $("#searchInput").val(), function(data) {
				page = nextPage;
			...
			ajaxRequest("GET", nextUrl, "page=" + 1 + "&bill_pay_type=" + bill_pay_type, function(data) {
				var res = JSON.parse(data);
				page = 1;
				showInfoOnTable(res);
				editOrDelete();
